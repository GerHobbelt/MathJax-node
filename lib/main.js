...
//
// The MathJaxPath is normalized against file:/// so that Windows paths are correct
//
var MathJaxPath = url.resolve("file:///","file:"+require.resolve('@gerhobbelt/mathjax'));
...
//
function GetWindow() {
  var obj = new JSDOM('', {userAgent: "Node.js"});
  assert(obj);
  window = obj.window;
  assert(window);
  ...
  window.MathJax = {
    ...
    AuthorInit: function () {
      ...
      MathJax.Hub.Register.StartupHook("SVG Jax Config",function () {
        ...
        //  use a monospace font and fake the size.  Since these
...
function GetMML(result) {
  ...
  try {
    ...
    return MathJax.Callback.After([GetMML, result], err.restart);
  }
  if (data.mml) result.mml = mml;
  if (data.mmlNode) result.mmlNode = new JSDOM(mml).body.firstChild;
}
...
function RerenderSVG(result) {
  if ((data.html || data.htmlNode || data.css) && (data.svg || data.svgNode)) {
    timer = setTimeout(RestartMathJax,data.timeout);
    var queue = MathJax.Callback.Queue(), $$ = Array;
    return queue.Push(
      $$(SetRenderer,"SVG"),
