...
var asciiMathSymbols = [];        // additional symbols to use in asciimath parser
...
function ConfigureMathJax() {
  window.MathJax = {
    ...
    AuthorInit: function () {
      ...
      //
      //  Set up None AsciiMath additional symbols
      //
      MathJax.Hub.Register.StartupHook("AsciiMath Jax Config",function() {
        var AM = MathJax.InputJax.AsciiMath.AM;
        const symbols = [];
        for(i = 0; i < asciiMathSymbols.length; ++i) {
          const symbol = Object.assign({}, asciiMathSymbols[i]);
...
function GetSVG(result) {
  ...
  if (data.speakText){
    ...
    // NOT ADDED TO SVG - sometimes content can have html entities which svg doesn't support
  }
...
exports.config = function (config) {
  ...
  if (config.asciiMathSymbols != null) {
    asciiMathSymbols = config.asciiMathSymbols;
  }
  if (config.MathJax) {
